...
const BookCard = ({ book }: { book: Book }) => {
  ...
  return (
    <div className="bg-card rounded-2xl border border-border shadow-sm card-hover overflow-hidden group">
      <div className={`${book.coverColor} p-6 flex items-center justify-center relative`}>
        <span className="text-6xl group-hover:animate-wiggle transition-transform">{book.coverEmoji}</span>
        <button
      ...
      <div className="p-4">
        ...
        <h3 className="font-display font-semibold text-sm leading-tight mb-1 line-clamp-1">{book.title}</h3>
        <p className="text-xs text-muted-foreground mb-1">by {book.author}</p>
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">{book.genre}</span>
