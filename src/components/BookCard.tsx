...
const BookCard = ({ book }: { book: Book }) => {
  ...
  return (
    <div className="bg-card rounded-xl border border-border shadow-sm card-hover overflow-hidden group">
      <div className={`${book.coverColor} p-6 flex items-center justify-center relative`}>
        <span className="text-5xl group-hover:scale-110 transition-transform duration-300">{book.coverEmoji}</span>
        <button
          onClick={(e) => { e.preventDefault(); toggleFavorite(book.id); }}
          className="absolute top-3 right-3 p-2 rounded-lg bg-card/80 backdrop-blur-sm hover:bg-card transition-colors"
        >
          <Heart className={`h-4 w-4 ${fav ? "fill-coral text-coral" : "text-muted-foreground"}`} />
        </button>
      </div>
      <div className="p-4">
        <div className="flex items-center gap-0.5 mb-1">
          {Array.from({ length: book.rating }).map((_, i) => (
            <Star key={i} className="h-3 w-3 fill-sunshine text-sunshine" />
          ))}
        ...
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-md font-medium">{book.genre}</span>
