...
import { books, categories, series as allSeries } from "@/data/books";

const ageGroups = ["All Ages", "8-13", "9-13", "10-14", "11-14", "11-15", "12-15"];
const sortOptions = ["Newest", "Most Popular", "Shortest", "Highest Rated"];
...
const BrowsePage = () => {
  ...
  let filtered = books.filter((b) => {
    ...
    const matchSeries = selectedSeries === "All" || b.series === selectedSeries;
    const matchAge = selectedAge === "All Ages" || b.ageRange === selectedAge;
    return matchSearch && matchGenre && matchSeries && matchAge;
  });
  ...
  if (sortBy === "Highest Rated") filtered.sort((a, b) => b.rating - a.rating);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="relative max-w-md mx-auto mb-8">
        ...
        <input
          type="text"
          placeholder="Search by title, author, character, or genre..."
          value={searchQuery}
      ...
      <div className="flex flex-wrap gap-4 mb-4 justify-center">
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {allSeries.map((s) => (
              <Button key={s.name} size="sm" variant={selectedSeries === s.name ? "default" : "outline"} onClick={() => setSelectedSeries(s.name)}>
                {s.name} ({s.count})
              </Button>
            ))}
          </div>
